    ...
    value: function setEvents() {
      ...
      this.tdTitleEl.forEach(function (el) {
        el.addEventListener("click", function () {
          ...
          _this.addCatButton.innerHTML = "Обновить";
      ...
      this.catNameInput.addEventListener("input", function () {
        if (_this.wasError) {
          _this.errorEl.classList.toggle("deactivated");
      ...
      this.addCatButton.addEventListener("click", function () {
        _this.addCatButton.disabled = true;
        var catName = document.querySelector(".cat_name").value;
        if (!catName) {
          _this.wasError = true;
          _this.errorEl.classList.toggle("deactivated");
          ...
          _this.addCat(catName).then(function (response) {
            ...
            if (response.data.error === "catexists") {
              _this.wasError = true;
              _this.errorEl.classList.toggle("deactivated");
              _this.errorEl.innerHTML = "Такая категория уже существует";
              _this.addCatButton.disabled = false;
            }
            if (response.data.error === "badinsert") {
              _this.wasError = true;
              _this.errorEl.classList.toggle("deactivated");
              _this.errorEl.innerHTML = "Что-то пошло не так :-( Перезагрузите страницу...";
            }
          })["catch"](function (error) {
            _this.wasError = true;
            _this.errorEl.classList.toggle("deactivated");
